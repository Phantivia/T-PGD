...
class MyBertSelfAttention(nn.Module):
    ...
    def p_init(self, input_length, batch_size = 1, init_mag = 1.0, cuda_device = 0, perturb_mask = None):
        ...
        self.perturb = torch.zeros((batch_size, input_length, self.config.hidden_size), device = torch.device('cuda:' + str(cuda_device))).uniform_(-init_mag, init_mag)/SQRT_NUMEL
    ...
    def p_accu(self, loss, adv_lr = 1.0, input_length = None, average_input_length = 20, project_to = None, perturb_mask = None):
        ...
        #Random test
        # SQRT_NUMEL = input_length * self.config.hidden_size
        # noise  = torch.zeros((1, input_length, self.config.hidden_size), device = torch.device('cuda:0' )).uniform_(-adv_lr, adv_lr)/SQRT_NUMEL
        # noise = torch.mul(perturb_mask.T, grad).detach()
        # noise = (adv_lr * noise/noise.norm()).detach()

        # grad = (grad + noise).detach()
